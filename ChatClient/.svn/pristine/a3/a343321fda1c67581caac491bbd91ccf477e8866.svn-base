package crypto;

import client.SecureChat;
import common.Utilities;
import java.util.Arrays;

/**
 * The class is used to implement the AES cryptographic algorithm.
 * @author Svetoslav Mihovski.
 * @version 2.0.
 * 
 *     Code for the class is adopted and modified from:
 *     https://n3vrax.wordpress.com/2011/08/14/aesrijndael-java-implementation/ (Accessed in March 2018)
 */
public class Aes {

  private static int Nb; // Plain text Block Size ( in words - word is 4 bytes or 32 bits)
  private static int Nk; // Key Block Size
  private static int Nr; // Number of Rounds, = Nk + 6
  private static byte[][] w; // expanded key

  private static int[] sbox = {
    
  };

  private static int[] inv_sbox = {
    
  };

  private static int[] Rcon = {
    
  };

  private static byte[] xor_func(byte[] a, byte[] b) {
    byte[] out = new byte[a.length];
    for (int i = 0; i < a.length; i++) {
      out[i] = (byte) (a[i] ^ b[i]);
    }
    return out;
  }

  private static byte[][] generateSubkeys(byte[] key) {
    byte[][] tmp = new byte[Nb * (Nr + 1)][4];

    int i = 0;
    while (i < Nk) {

      tmp[i][0] = key[i * 4];
      tmp[i][1] = key[i * 4 + 1];
      tmp[i][2] = key[i * 4 + 2];
      tmp[i][3] = key[i * 4 + 3];
      i++;
    }
    i = Nk;
    while (i < Nb * (Nr + 1)) {
      byte[] temp = new byte[4];
      for (int k = 0; k < 4; k++) {
        temp[k] = tmp[i - 1][k];
      }
      if (i % Nk == 0) {
        temp = subWord(rotateWord(temp));
        temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
      } else if (Nk > 6 && i % Nk == 4) {
        temp = subWord(temp);
      }
      tmp[i] = xor_func(tmp[i - Nk], temp);
      i++;
    }

    return tmp;
  }

  private static byte[] subWord(byte[] in) {
    byte[] tmp = new byte[in.length];
    for (int i = 0; i < tmp.length; i++) {
      tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);
    }
    return tmp;
  }

  private static byte[] rotateWord(byte[] input) {
    byte[] tmp = new byte[input.length];
    tmp[0] = input[1];
    tmp[1] = input[2];
    tmp[2] = input[3];
    tmp[3] = input[0];

    return tmp;
  }

  private static byte[][] addRoundKey(byte[][] state, byte[][] w, int round) {

    byte[][] tmp = new byte[state.length][state[0].length];

    for (int c = 0; c < Nb; c++) {
      for (int l = 0; l < 4; l++) {
        tmp[l][c] = (byte) (state[l][c] ^ w[round * Nb + c][l]);
      }
    }

    return tmp;
  }

  private static byte[][] subBytes(byte[][] state) {

    byte[][] tmp = new byte[state.length][state[0].length];
    for (int row = 0; row < 4; row++) {
      for (int col = 0; col < Nb; col++) {
        tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);
      }
    }
    return tmp;
  }

  private static byte[][] invSubBytes(byte[][] state) {
    for (int row = 0; row < 4; row++) {
      for (int col = 0; col < Nb; col++) {
        state[row][col] = (byte) (inv_sbox[(state[row][col] & 0x000000ff)] & 0xff);
      }
    }
    return state;
  }

  private static byte[][] shiftRows(byte[][] state) {

    byte[] t = new byte[4];
    for (int r = 1; r < 4; r++) {
      for (int c = 0; c < Nb; c++) {
        t[c] = state[r][(c + r) % Nb];
      }
      for (int c = 0; c < Nb; c++) {
        state[r][c] = t[c];
      }
    }

    return state;
  }

  private static byte[][] invShiftRows(byte[][] state) {
    byte[] t = new byte[4];
    for (int r = 1; r < 4; r++) {
      for (int c = 0; c < Nb; c++) {
        t[(c + r) % Nb] = state[r][c];
      }
      for (int c = 0; c < Nb; c++) {
        state[r][c] = t[c];
      }
    }
    return state;
  }

  private static byte[][] invMixColumns(byte[][] s) {
    int[] sp = new int[4];
    byte b02 = (byte) 0x0e;
    byte b03 = (byte) 0x0b;
    byte b04 = (byte) 0x0d;
    byte b05 = (byte) 0x09;
    for (int c = 0; c < 4; c++) {
      sp[0] = ffMul(b02, s[0][c]) ^ ffMul(b03, s[1][c]) ^ ffMul(b04, s[2][c]) ^ ffMul(b05, s[3][c]);
      sp[1] = ffMul(b05, s[0][c]) ^ ffMul(b02, s[1][c]) ^ ffMul(b03, s[2][c]) ^ ffMul(b04, s[3][c]);
      sp[2] = ffMul(b04, s[0][c]) ^ ffMul(b05, s[1][c]) ^ ffMul(b02, s[2][c]) ^ ffMul(b03, s[3][c]);
      sp[3] = ffMul(b03, s[0][c]) ^ ffMul(b04, s[1][c]) ^ ffMul(b05, s[2][c]) ^ ffMul(b02, s[3][c]);
      for (int i = 0; i < 4; i++) {
        s[i][c] = (byte) (sp[i]);
      }
    }

    return s;
  }

  private static byte[][] mixColumns(byte[][] s) {
    int[] sp = new int[4];
    byte b02 = (byte) 0x02;
    byte b03 = (byte) 0x03;
    for (int c = 0; c < 4; c++) {
      sp[0] = ffMul(b02, s[0][c]) ^ ffMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
      sp[1] = s[0][c] ^ ffMul(b02, s[1][c]) ^ ffMul(b03, s[2][c]) ^ s[3][c];
      sp[2] = s[0][c] ^ s[1][c] ^ ffMul(b02, s[2][c]) ^ ffMul(b03, s[3][c]);
      sp[3] = ffMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ ffMul(b02, s[3][c]);
      for (int i = 0; i < 4; i++) {
        s[i][c] = (byte) (sp[i]);
      }
    }

    return s;
  }

  private static byte ffMul(byte a, byte b) {
    byte aa = a;
    byte bb = b;
    byte r = 0;
    byte t;
    while (aa != 0) {
      if ((aa & 1) != 0) {
        r = (byte) (r ^ bb);
      }
      t = (byte) (bb & 0x80);
      bb = (byte) (bb << 1);
      if (t != 0) {
        bb = (byte) (bb ^ 0x1b);
      }
      aa = (byte) ((aa & 0xff) >> 1);
    }
    return r;
  }

  private static byte[] encryptBloc(byte[] in) {

    byte[][] state = new byte[4][Nb];

    for (int i = 0; i < in.length; i++) {
      state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
    }

    state = addRoundKey(state, w, 0);
    for (int round = 1; round < Nr; round++) {
      state = subBytes(state);
      state = shiftRows(state);
      state = mixColumns(state);
      state = addRoundKey(state, w, round);
    }
    state = subBytes(state);
    state = shiftRows(state);
    state = addRoundKey(state, w, Nr);

    byte[] tmp = new byte[in.length];
    for (int i = 0; i < tmp.length; i++) {
      tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
    }

    return tmp;
  }

  private static byte[] decryptBloc(byte[] in) {

    byte[][] state = new byte[4][Nb];
    for (int i = 0; i < in.length; i++) {
      state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
    }
    state = addRoundKey(state, w, Nr);

    for (int round = Nr - 1; round >= 1; round--) {
      state = invSubBytes(state);
      state = invShiftRows(state);
      state = addRoundKey(state, w, round);
      state = invMixColumns(state);
    }
    state = invSubBytes(state);
    state = invShiftRows(state);
    state = addRoundKey(state, w, 0);

    byte[] tmp = new byte[in.length];
    for (int i = 0; i < tmp.length; i++) {
      tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
    }

    return tmp;
  }

  /** This method is used to make AES encrytion in CBC mode.
   * @param in ; plaintext as byte array
   * @param key ; AES key
   * @return encrypted message as byte array
   */
  public static byte[] encryptCbc(byte[] in, byte[] key) {

    Nb = 4;
    Nk = key.length / 4;
    Nr = Nk + 6;

    byte[] initIv =
        Utilities.getSessionKey(SecureChat.SESSION_KEY_LENGTH); // generate iv for CBC mode
    byte[] iv = initIv.clone(); // initialize work IV

    int lenght = 0;
    byte[] padding = new byte[1];

    lenght = 16 - in.length % 16;
    padding = new byte[lenght];
    padding[0] = (byte) 0x80;

    int i;
    for (i = 1; i < lenght; i++) {
      padding[i] = 0;
    }

    byte[] tmp = new byte[in.length + lenght];
    byte[] bloc = new byte[16];

    w = generateSubkeys(key);

    int count = 0;

    for (i = 0; i < in.length + lenght; i++) {
      if (i > 0 && i % 16 == 0) {
        bloc = xor_func(bloc, iv); // iv XOR block
        bloc = encryptBloc(bloc);
        iv = bloc.clone(); // iv for next XOR
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
      }
      if (i < in.length) {
        bloc[i % 16] = in[i];
      } else {
        bloc[i % 16] = padding[count % 16];
        count++;
      }
    }
    if (bloc.length == 16) {
      bloc = xor_func(bloc, iv); // iv XOR block
      bloc = encryptBloc(bloc);
      System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
    }

    return Utilities.concByteAreas(initIv, tmp); // add iv to encrypted message/file
  }

  /** This method is used to make AES decryption in CBC mode.
   * @param in ; ciphertext as byte array
   * @param key ; AES key
   * @return decrypted message as byte array
   */
  public static byte[] decryptCbc(byte[] in, byte[] key) {

    byte[] tmp = new byte[in.length];
    byte[] bloc = new byte[16];

    byte[] iv = Arrays.copyOfRange(in, 0, (SecureChat.SESSION_KEY_LENGTH / 8)); // extract iv
    in =
        Arrays.copyOfRange(
            in, (SecureChat.SESSION_KEY_LENGTH / 8), in.length); // extract message/file
    byte[] ivtemp = new byte[16]; // temporal iv

    Nb = 4;
    Nk = key.length / 4;
    Nr = Nk + 6;
    w = generateSubkeys(key);

    int i;
    for (i = 0; i < in.length; i++) {
      if (i > 0 && i % 16 == 0) {
        ivtemp = bloc.clone(); // store encrypted block as iv
        bloc = decryptBloc(bloc);
        bloc = xor_func(bloc, iv); // iv XOR block
        iv = ivtemp;
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
      }
      if (i < in.length) {
        bloc[i % 16] = in[i];
      }
    }
    bloc = decryptBloc(bloc);
    bloc = xor_func(bloc, iv); // iv XOR block
    System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);

    tmp = deletePadding(tmp);

    return tmp;
  }

  private static byte[] deletePadding(byte[] input) {
    int count = 0;

    int i = input.length - 1;
    while (input[i] == 0) {
      count++;
      i--;
    }

    byte[] tmp = new byte[input.length - count - 1];
    System.arraycopy(input, 0, tmp, 0, tmp.length);
    return tmp;
  }
}
